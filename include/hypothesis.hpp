...
#define threshold 0.000000000000000000000001f
...
void hypothesis<thetaT>::train(dataTable<T, rows, labels, outputs>& data_table, unsigned mapToOutput)
{
	...
	do
	{
		...
		//deviation is mathematical sense is the difference
		...
		//
		//I noticed that every iteration's calculation required
		//multiple use of the value of deviation of each row so,
		//I separated it and created a lookup table so as to 
		//not have to repeat calculation of it again
		...
		//This is equivalent to simultaneously updating
		//all weights as delta has already been calculated
		//with all of the previous theta values so,
		//assume deviations as constants from this point
		for(unsigned j = 0; j<data_table.m_xColumns; j++)
		{
